...
//Last Modified: June 12th 2014
//made the plugin Visual Composer Ready
//small layout improvements
//Added text labels mode
//Added custom css field
//Small URL fixes (https)
...
function build_i_world_map_exec($id,$type) {
   ...
   $new_iwm_array = array(
	...
	if($type == "shortcode") {
		return $beforediv."<div id='iwm_map_canvas'><div id='map_canvas_".$id."' class='i_world_map'>".$placeholder."</div></div>".$afterdiv;
	} else {
		echo $beforediv."<div id='iwm_map_canvas'><div id='map_canvas_".$id."' class='i_world_map'>".$placeholder."</div></div>".$afterdiv;
	}
}
...
function imap_include_responsive_js() {
	...
	wp_register_script( 'imapresponsive', plugins_url( 'js/responsive.js', __FILE__ ),array('jquery','iwmjs'),false,false);
...
function i_world_map_includes_add() {
    ...
    wp_register_script( 'iwjscolor', plugins_url( 'js/jscolor.js' , __FILE__ ) );
	wp_register_script( 'iwjsadmin', plugins_url( 'js/admin.js' , __FILE__ ) );	 
...
function i_world_map_scripts($iwmparam_array) {
	...
	wp_register_script( 'iwmjs', plugins_url( '/js/shortcode.js', __FILE__) , array('jsapifull') , false, false);
    ...
    <table width="100%" border="0" cellspacing="5" cellpadding="5">
      <tr>
        <td width="25%" valign="top"><h3>Details
          ...
          <table width="100%" border="0" cellspacing="2" cellpadding="2" id="name-table">
          <tr valign="top">
            <td>Name <br><input type="text" name="name" value="<?php echo $name; ?>" /></td>
          </tr>
          <tr valign="top">
            
            <td>Description<br><textarea name="description" cols="20" rows="3"><?php echo $description; ?></textarea></td>
          </tr>
          ...
          <table>
            ...
            <tr valign="top" >
             ...
             <tr valign="top" >
              ...
              <td><span class="howto">The settings bellow will be ignored, since the Responsive Mode is ON.</span></td>
            </tr>
        ...
        <td width="75%" valign="top"><h3>Map Settings</h3>
          <table width="100%" border="0" cellspacing="5" cellpadding="5" id="add-table">
            <tr valign="top" >
              ...
              <span class="howto">Choose the region you want to display the map to focus on</span>
          		</td>
            ...
            <tr valign="top" >
              ...
              <td>
                <select name="display_mode" onchange="isolinkcheck();">
                  ...
                  <option value="markers02"  <?php if($display_mode=='markers02') { ?>selected="selected" <?php } ?>>Round Markers (Coordinates Code)</option>
                  <option value="text"  <?php if($display_mode=='text') { ?>selected="selected" <?php } ?> >Text Labels</option>
                  <option value="text02"  <?php if($display_mode=='text02') { ?>selected="selected" <?php } ?>>Text Labels (Coordinates Code)</option>
              </select></td>
            ...
            <tr> <td colspan="3"><input name="interactive" type="checkbox"  id="interactive" onchange="drawVisualization();" value="1" <?php if($interactive==1) { ?>checked <?php } ?> /> Enable Region Hover effect
                ...
                <br />
                  <input name="tooltipt" type="checkbox"  id="tooltipt" onchange="drawVisualization();" value="1" <?php if($tooltipt==1) { ?>checked <?php } ?> />
Show Tooltip on Hover (In Regions Mode hover effect must be active)
</td> </tr> 
...
<span class="iwmsmall">The HTML tooltip might look different on you site since it can inherit CSS rules from your theme.</span>
 <?php } ?>
       ...
       <p>
        <span class="iwmsmall">If you want to make advanced CSS customizations to the output of your map, <a href="http://cmoreira.net/responsive-interactive-maps/custom-css/" target="_blank">check this page on the official plugin website</a>. <br>You can place any extra CSS in the 'Custom CSS' option in the plugin settings page.</span>
        </p>
      ...
      <tr>
        <td width="25%"><form method="post" action="options.php" id="dsform">
          ...
          <p><strong>HTML Tooltips (Beta Feature)</strong></p>
              ...
              <span class="howto">This is a feature still in beta. Consider that the tooltip will inherit styles from your theme that might affect the way the tooltip displays. You can create custom CSS editing the styles.css of the plugin folder.</span> 
              </p>
          </td></tr>
