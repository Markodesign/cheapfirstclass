...
qf.Validator = function(form, rules)
{
    ...
    ***************************************/
    ...
    for (var i = 0, rule; rule = this.rules[i]; i++) {
        ...
        name = $('#'+rule.owner).attr('name');
        qf2_options.rules[name] = {};
        qf2_options.messages[name] = {};
    }
    for (var i = 0, rule; rule = this.rules[i]; i++) {
        if (this.errors.hasKey(rule.owner)) {
            continue;
        ...
        name = $('#'+rule.owner).attr('name');
        method_name = name+i;
        $.validator.addMethod(method_name, rule.callback);
        qf2_options.rules[name][method_name] = true;
        qf2_options.messages[name][method_name] = rule.message;
    }
        
//    var validator = $("form#"+form_id).validate(qf2_options);
//
//    datePickSettings = {
//        startDate: '01-01-2000',
//        endDate:   '01-01-2015'
//    };
//    $(".datepicker").datepicker(datePickSettings)
...
//	});
    ...
    $('textarea.ckeditorBasic').ckeditor(ckeditorBaasic);


    ckeditor_names = [];
    $('textarea.ckeditorBasic').each(function () {
        id = $(this).attr('id');
        name = $(this).attr('id');
        ...
        editor.on("blur", function(e) {
            ...
            if ( name in validator.submitted ) {
                validator.element("#"+id);
            }
        });
    })
